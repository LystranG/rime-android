...
**中英文切换（重要）**：
* 在 17/18 键上切英文：按**回车左边的「英」键** → 26 键英文键盘（并自动切到英文模式）；
* 在英文键盘切回中文：按**回车左边的「中」键** → 自动回到四叶草 18 键（双拼方案下）；
* ⚠️ 不要用顶栏的「中/英」切换按钮：Trime 的中英切换**不会自动换键盘**（官方行为），
  在 18 键上直接切英文会因为缺字母打不全。
* 持久化：主题里有一个与双拼方案同名的隐形键盘（`double_pinyin_flypy`），
  部署后/切方案/英文返回默认，只要双拼方案生效，四叶草 18 键就是默认键盘。
