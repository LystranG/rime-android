...
├── 简纯+.trime.custom.yaml       ← 本目录（17 键键盘 + 功能键盘按钮）
...
布局切换入口：**长按回车 → 功能键盘**，第 3 行有 `26键 / 17键 / 18键 / 14键` 按钮（17键是本包挂上去的）。

| 键盘 | 配合的方案 | 原理 |
| --- | --- | --- |
| 26 键（主题自带） | 全拼 / 双拼都能用 | 标准键盘 |
| 18 键（主题自带） | **薄荷全拼 rime_mint** | 万象官方同款 8 组合并（W E→w、R T→r、I O→i、S D→s、F G→f、J K→j、X C→x、B N→b），映射在 `rime_mint.custom.yaml` |
| 17 键（本包新增） | **小鹤双拼 double_pinyin_flypy** | 四叶草 9 组合并（QW/ER/TY/OP/AS/DF/JK/ZX/BN），映射在 `double_pinyin_flypy.custom.yaml` |
| 14 键（主题自带） | 本包未启用 | 没加对应映射，按了也打不全，忽略 |

> ⚠️ 共键映射跟方案走：18 键在双拼下会缺字母键、17 键在全拼下会缺字母键——**先切对方案，再切键盘**。
>
> ✅ 映射用的是 `derive`（并集），**26 键永远可用**：开了 18/17 键映射后 26 键照常打字，
> 合并键上两类候选并存（比如 18 键的 `S D` 键 = s、d 的词都出）。这和万象原生的 `xlit`
> （替换式、切布局要改配置重部署）不同，是刻意选择的。
>
> 想让小鹤双拼默认就用 17 键：把 `简纯+.trime.custom.yaml` 里
> `"preset_keyboards/17jian"` 改名为 `"preset_keyboards/double_pinyin_flypy"` 即可
> （Trime 按「键盘 ID = 方案 ID」自动套用）。
...
* **func 键盘按钮**：`简纯+.trime.custom.yaml` 用 `keys/@10/` 把功能键盘行尾占位键替换成 17 键按钮；
  万象官方以后若自带 17 键，删掉该段即可。若 Trime 版本差异导致按钮位置不对，按实际
  顺序调整索引（该文件里有注释）。
...
2. 长按回车 → 功能键盘 → 17键按钮出现且能切换，17 键键盘样式与主题一致（圆角图片键）；
