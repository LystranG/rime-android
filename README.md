...
**功能键盘（长按回车）里的「同步」按钮**：一键触发 Rime 的「同步用户数据」
（userdb 导出/合并到 `sync/`），配合 Syncthing 词库同步用，不用离开当前应用。
原「设置」按钮已移除——它会以特殊方式启动 Trime 设置页，部分系统上**无法返回**
（Trime 自身的坑）；需要改 Trime 设置请从桌面图标打开 Trime 应用。
