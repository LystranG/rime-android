...
**中英文切换（已优化，重要）**：
* **顶栏按钮（「英」）**：从任何键盘点击 → 直接跳到 **26 键英文**键盘（并强制英文模式）；
* **英文键盘 → 中文**：按**回车左边的「中」键** → 双拼方案下回到四叶草 18 键（全拼回 26 键）；
* 17/18 键与英文键盘都写了 `reset_ascii_mode`：每次显示键盘自动重置中英状态，
  不会再出现「英文键盘是中文」「中文键盘残留英文」的错乱；
* ⚠️ Trime 的限制：`Mode_switch` 类切换只改模式不换键盘（源码行为），所以顶栏按钮被
  改成了「跳转英文键盘」而不是原地的中英切换——原地在共键布局上切英文必然缺字母。
