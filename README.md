...
**切换方式**：① 长按回车 → 功能键盘，第 3 行 `17键 | 26键 | 18键 | 14键 | 鹤18`；② Trime 键盘切换列表（本包已把 17/18 键追加进主题的 `style/keyboards`，与主题自带的 26/18/14 同一列表直接切换）。
...
| 小鹤 18 键（本包，hint「鹤18」） | **小鹤双拼 double_pinyin_flypy** | 同分组拆开 B/N；`你好 = BN IO H C` 四键直出 |
...
2. 长按回车 → 功能键盘 → 17键/鹤18 按钮出现且能切换；Trime 键盘切换列表里也能看到「小鹤双拼·17键 / 18键」；
