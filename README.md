...
**中英文切换（重要）**：
* 顶栏原来的「中/英」按钮已**移除**（Trime 的中英切换只改模式不换键盘，在共键布局上切英文必然缺字母）；
* 顶栏按钮组已用透明占位键**近似居中**（Trime 源码把顶栏按钮写死为靠右排列，无居中选项）；
  想调位置改 `简纯+.trime.yaml` 里 `tool_bar/spacer_button` 的 `size` 第一个数字
  （越大越往中间；窄屏/横屏若按钮换行就调小）。
* **在 17/18 键切英文**：按**回车左边的「英」键** → 26 键英文键盘（强制英文模式）；
* **在英文键盘切回中文**：按**回车左边的「中」键** → 双拼方案下回到四叶草 18 键（全拼回 26 键）；
* 17/18 键与英文键盘都写了 `reset_ascii_mode`：每次显示键盘自动重置中英状态，
  不会再出现「英文键盘是中文」「中文键盘残留英文」的错乱；
* ⚠️ Trime 限制：`Mode_switch` 类切换只改模式不换键盘（源码行为），所以共键布局上
  不要用任何「原地中英切换」，一律走「英/中」键盘跳转键。
...
**功能键盘（长按回车）**：主页 / 方案 / 设置 / 部署 / 主题 / 颜色 / 26·18·14 键切换。
⚠️ 其中的「设置」按钮在部分机型上会打开一个**无法返回**的 Trime 设置页（Trime 自身
从输入法启动设置页的导航缺陷）——遇到就切走应用再切回来；改设置更稳妥的方式是
从桌面图标打开 Trime 应用。
...
6. 密码框等强制英文场景自动变英文 26 键；英文键盘回车左边有「中」键可回四叶草 18 键；
